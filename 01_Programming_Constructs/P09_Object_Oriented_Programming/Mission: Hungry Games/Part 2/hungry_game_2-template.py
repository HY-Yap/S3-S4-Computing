...
class Food(Thing):
    def __init__(self, name, food_value):
        super().__init__(name)
        self.food_vlaue = food_value
    
    def get_food_value(self):
        return self.food_vlaue
...
class Animal(LivingThing):
    ...
    def go_to_heaven(self):
        food_obj = Food(f"{self.name} meat", self.food_value)
        self.get_place().add_object(food_obj)
...
class Tribute(Person):
    ...
    def eat(self, food):
        if food in self.inventory:
            ...
            self.inventory.pop(self.inventory.index(food))
    ...
    def get_weapons(self):
        ...
        for item in self.inventory:
            if isinstance(item, Weapon):
                weapon_lst.append(item)
        return tuple(weapon_lst)
    ...
    def attack(self, living_thing: LivingThing, weapon: Weapon):
        self.living_thing = living_thing
        self.weapon = weapon
        if self.living_thing.get_place() == self.get_place() and self.weapon in self.inventory:
            self.living_thing.reduce_health(weapon.damage())
